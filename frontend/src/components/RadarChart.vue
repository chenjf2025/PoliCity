  ...
  const option = {
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        // 显示全部6个维度的得分
        let html = `<strong>${params.name || params.seriesName}</strong><br/>`;
        props.dimensions.forEach((dim, idx) => {
          const score = values[idx] ?? '--';
          html += `${dim.name}: <strong>${score}</strong>分`;
          html += ` (权重${(dim.weight * 100).toFixed(0)}%)<br/>`;
        });

        if (props.sourceInfo) {
          html += `<br/>来源: ${props.sourceInfo.source_name || '-'}`;
          if (props.sourceInfo.source_url) {
            html += ` <a href="${props.sourceInfo.source_url}" target="_blank" style="color:#409eff">查看来源</a>`;
          }
        }
        return html;
      }
